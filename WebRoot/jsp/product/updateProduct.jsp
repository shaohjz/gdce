			...
			<div class="col-md-12 column">
				<div class="row clearfix">
					...
					<div class="col-md-10 column">
						...
						%>
							<form action="<%=context%>/product?forward=update" method="post">
								</style>
								<tr>
									...
									<select style="width: 200px; height: 30px;" name="productType">
										<option  id="productType"
											selected="selected"><%=product.getProductType()%></option>
										<option>烟煤</option>
										<option>混煤</option>
										<option>电煤</option>
										<option>褐煤</option>
								...
								<tr>
									<select style="width: 200px; height: 30px;" name="destAddress">
										<option  id="destAddress"
											selected="selected"><%=product.getDestAddress()%></option>
										<option>集宁</option>
										<option>达州电厂</option>
										<option>康平</option>
										<option>酒泉</option>
								...
								<p>
									<span style="margin-top: 40px; margin-left: 15%;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;产地：
										...
										<option value="srcAddress" id="srcAddress" selected="selected"><%=product.getSrcAddress()%></option>
										<option>山西</option>
										<option>内蒙</option>
									</select><span style="margin-left: 20px;">交货方式：</span> <select
										style="width: 200px; height: 30px;" name="transactionMode">
										<option  id="transactionMode"
											selected="selected"><%=product.getTransactionMode()%></option>
										<option>场地交货</option>
										<option>到厂交货</option>
									</select>
								</p>
								<table style="margin-left: 15%;">
									<tr>
										...
										<td><input name="price" type="text" id="price"
											...
											maxlength="5" value=<%=product.getPrice()%>
										...
										<td><input name="quantity" type="text" id="quantity"
											style="width: 200px; height: 30px; margin-top: 40px;" value=<%=product.getQuantity()%>
											maxlength="5"
											onkeyup="if(! /^d*(?:d{0,2})?$/.test(this.value)){alter('只能输入数字，只保留两位小数');this.value='';}"></td>
									...
									<tr>
										...
										<td><input name="lowPower" type="number" id="lowPower"
											style="width: 200px; height: 30px; margin-top: 40px;"  value=<%=product.getLowPower()%>
											max="99999"></td>
										...
										<td><input name="volatiles" type="text" id="volatiles"
											...
											style="width: 200px; height: 30px; margin-top: 40px;"  value=<%=product.getVolatiles()%>
											maxlength="5"
											onkeyup="if(! /^d*(?:d{0,2})?$/.test(this.value)){alter('只能输入数字，只保留两位小数');this.value='';}"></td>
									</tr>
									<tr>
										...
										<td><input name="fullMoisture" type="text"
											...
											max="100" value=<%=product.getFullMoisture()%>
											style="width: 200px; height: 30px; margin-top: 40px;"
											maxlength="5"
											onkeyup="if(! /^d*(?:d{0,2})?$/.test(this.value)){alter('只能输入数字，只保留两位小数');this.value='';}"></td>
										...
										<td><input name="fullSulfur" type="text" id="fullSulfur"
											placeholder="请输入1-100以内数字" min="0" max="100"
											style="width: 200px; height: 30px; margin-top: 40px;"
											maxlength="5" value=<%=product.getFullSulfur()%>
											onkeyup="if(! /^d*(?:d{0,2})?$/.test(this.value)){alter('只能输入数字，只保留两位小数');this.value='';}"></td>
									</tr>
									<tr>

										<td><div align="right" style="margin-top: 40px;">截止时间：</div></td>
										<td><input name="endDate" type="date" id="endDate" value=<%=product.getEndDate()%>
								...
								<table style="margin-left: 15%;">
									<tr>
										...
										<td><textarea name="remark" id="remark" value=<%=product.getRemark()%>
